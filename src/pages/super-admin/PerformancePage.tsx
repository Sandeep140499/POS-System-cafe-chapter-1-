...
export default function PerformancePage() {
  ...
  const filtered = useMemo(() => {
    let list = employees.filter((e) => e.name.toLowerCase().includes(search.toLowerCase()) || e.id.toLowerCase().includes(search.toLowerCase()));
    list = [...list].sort((a, b) => {
  ...
  const downloadCSV = () => {
    const rows = ["Rank,Name,ID,Orders,Revenue,Target%", ...ranked.map((e) => `${e.rank},${e.name},${e.id},${e.orders},${e.revenueRaw},${e.target}`)];
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2, 1fr)", md: "repeat(3, 1fr)" }, gap: 2, mb: 3 }}>
        {stats.map((s) => (
          <SectionPaperCard key={s.label}>
            <Typography sx={{ fontSize: 10.5, fontWeight: 700, color: "#c86030", letterSpacing: "0.1em", textTransform: "uppercase", mb: 1, fontFamily: "Inter, sans-serif" }}>
              {s.label}
            </Typography>
            <Typography sx={{ fontSize: 30, fontWeight: 700, color: "#1a1a1a", lineHeight: 1, fontFamily: "Inter, sans-serif" }}>{s.value}</Typography>
            <Typography sx={{ fontSize: 12, color: "#999", mt: 0.5, fontFamily: "Inter, sans-serif" }}>{s.sub}</Typography>
          </SectionPaperCard>
        ))}
      </Box>
                ...
                <TableRow key={e.id} sx={{ "&:hover": { bgcolor: "#faf9f7" }, "&:last-child td": { border: 0 } }}>
                  ...
                  <TableCell sx={cSx}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                      <Avatar sx={{ width: 32, height: 32, bgcolor: avatarColor(e.name), fontSize: 11, fontWeight: 700 }}>
                        {initials(e.name)}
                      </Avatar>
                      <Box>
                  ...
                  <TableCell sx={{ ...cSx, textAlign: "center" }}>
                    <Typography sx={{ fontSize: 14, fontWeight: 500, color: "#1a1a1a", fontFamily: "Inter, sans-serif" }}>{e.orders}</Typography>
                  </TableCell>
                  <TableCell sx={{ ...cSx, textAlign: "right" }}>
                    <Typography sx={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", fontFamily: "Inter, sans-serif" }}>{formatINR(e.revenueRaw)}</Typography>
                  </TableCell>
                  <TableCell sx={{ ...cSx, textAlign: "center" }}>
                    <Typography sx={{ fontSize: 14, fontWeight: 500, color: e.target >= 70 ? "#2e7d32" : e.target >= 40 ? "#c86030" : "#999", fontFamily: "Inter, sans-serif" }}>
                      {e.target}%
                    </Typography>
                  </TableCell>
                  <TableCell sx={cSx}>
