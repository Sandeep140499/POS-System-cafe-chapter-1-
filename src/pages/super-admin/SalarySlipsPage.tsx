...
import { Box, Typography, Button, Chip, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Select, MenuItem, FormControl, InputLabel, IconButton, Dialog, DialogContent, DialogTitle, Fade } from "@mui/material";
...
const hSx = { fontSize:10, fontWeight:700, color:"#999", letterSpacing:"0.08em", textTransform:"uppercase" as const, borderBottom:"1px solid #eae7e2", py:1.5, fontFamily:"Josefin Sans, sans-serif" };
...
const COMPANY_INFO = {
  ...
  get logoUrl() { return localStorage.getItem("branch_logo_url") || ""; }
};
...
const calcSalary = (slip: Slip) => {
  const grossEarnings = slip.basic + slip.allowances.reduce((sum, a) => sum + a.amount, 0) + slip.overtimeAmount;
  const totalDeductions = slip.deductions.reduce((sum, d) => sum + d.amount, 0) + slip.lateDeductions;
...
<body>
  ...
  return (
    <Box>
      {toast && (
        <Box sx={{ position: "fixed", top: 80, right: 24, zIndex: 9999, bgcolor: "#1a1a1a", color: "#fff", px: 3, py: 1.5, borderRadius: "12px", fontSize: 14, fontFamily: "Josefin Sans, sans-serif" }}>
          {toast}
        </Box>
      )}
      
      <PageHeader label="FINANCE" title="Salary Slips" subtitle="Generate, preview, and send employee payslips." action={
        <Button 
          startIcon={<AddRoundedIcon />} 
          onClick={() => { resetForm(); setModalOpen(true); }} 
          sx={{ bgcolor: "#c86030", color: "#fff", borderRadius: "20px", px: 2.5, py: 1, textTransform: "none", fontSize: 14, fontFamily: "Josefin Sans, sans-serif", "&:hover": { bgcolor: "#a8502a" } }}
        >
          Generate Slip
        </Button>
      } />
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, mb: 3 }}>
        <Box sx={{ bgcolor: "#fff", borderRadius: "20px", border: "1px solid #eae7e2", p: 2.5 }}>
          <Typography sx={{ fontSize: 10.5, fontWeight: 700, color: "#c86030", letterSpacing: "0.1em", textTransform: "uppercase", mb: 0.5, fontFamily: "Josefin Sans, sans-serif" }}>
            TOTAL SALARY PAID (THIS MONTH)
          </Typography>
          <Typography sx={{ fontSize: 30, fontWeight: 700, color: "#1a1a1a", fontFamily: "Josefin Sans, sans-serif" }}>
            {f(totalPaid)}
          </Typography>
        </Box>
        <Box sx={{ bgcolor: "#fff", borderRadius: "20px", border: "1px solid #eae7e2", p: 2.5 }}>
      ...
      <Box sx={{ bgcolor: "#fff", borderRadius: "20px", border: "1px solid #eae7e2", overflow: "hidden" }}>
        {slips.length === 0 ? (
          <Box sx={{ textAlign: "center", py: 8 }}>
            ...
            <Typography sx={{ fontSize: 15, color: "#999", fontFamily: "Josefin Sans, sans-serif" }}>No salary slips generated yet.</Typography>
          </Box>
        ) : (
          <TableContainer>
            <Table>
              ...
              <TableBody>
                {slips.map(sl => {
                  ...
                  return (
                    <TableRow key={sl.id} sx={{ "&:hover": { bgcolor: "#faf9f7" }, "&:last-child td": { border: 0 } }}>
                      <TableCell sx={cSx}>
                        <Typography sx={{ fontSize: 13.5, fontWeight: 600, color: "#c86030", fontFamily: "Josefin Sans, sans-serif" }}>{sl.slipNo}</Typography>
                      </TableCell>
                      <TableCell sx={cSx}>
                        <Typography sx={{ fontSize: 14, fontWeight: 500, color: "#1a1a1a", fontFamily: "Josefin Sans, sans-serif" }}>{sl.employee}</Typography>
                      </TableCell>
                      <TableCell sx={cSx}>
                        <Typography sx={{ fontSize: 13.5, color: "#555", fontFamily: "Josefin Sans, sans-serif" }}>{sl.period}</Typography>
                      </TableCell>
                      <TableCell sx={{ ...cSx, textAlign: "right" }}>
                        <Typography sx={{ fontSize: 14, color: "#333", fontFamily: "Josefin Sans, sans-serif" }}>{f(calc.grossEarnings)}</Typography>
                      </TableCell>
                      <TableCell sx={{ ...cSx, textAlign: "right" }}>
                        <Typography sx={{ fontSize: 14, color: "#c62828", fontFamily: "Josefin Sans, sans-serif" }}>-{f(calc.totalDeductions)}</Typography>
                      </TableCell>
                      <TableCell sx={{ ...cSx, textAlign: "right" }}>
                        <Typography sx={{ fontSize: 15, fontWeight: 700, color: "#2e7d32", fontFamily: "Josefin Sans, sans-serif" }}>{f(calc.netSalary)}</Typography>
                      </TableCell>
                      <TableCell sx={cSx}>
                        <Chip 
                          label={sl.status} 
                          size="small" 
                          sx={{ bgcolor: sl.status === "Sent" ? "#e3f0e8" : "#eeedea", color: sl.status === "Sent" ? "#2e7d32" : "#555", fontWeight: 600, fontSize: 11, height: 24, borderRadius: "12px" }} 
                        />
                      </TableCell>
                      <TableCell sx={{ ...cSx, textAlign: "center" }}>
                        <Box sx={{ display: "flex", justifyContent: "center", gap: 0.5 }}>
                          <IconButton size="small" onClick={() => handlePreview(sl)} sx={{ color: "#888", "&:hover": { color: "#c86030", bgcolor: "#fef3f0" } }}>
                            <VisibilityOutlinedIcon sx={{ fontSize: 17 }} />
                          </IconButton>
                          <IconButton size="small" onClick={() => handleDownload(sl)} sx={{ color: "#888", "&:hover": { color: "#1565c0", bgcolor: "#e8f0fe" } }}>
                            <DownloadOutlinedIcon sx={{ fontSize: 17 }} />
                          </IconButton>
                          <IconButton size="small" onClick={() => { setSlips(p => p.map(s => s.id === sl.id ? { ...s, status: "Sent" } : s)); showToast("Email sent!"); }} sx={{ color: "#888", "&:hover": { color: "#2e7d32", bgcolor: "#e3f0e8" } }}>
                            <EmailOutlinedIcon sx={{ fontSize: 17 }} />
                          </IconButton>
                          <IconButton size="small" onClick={() => showToast("WhatsApp message prepared!")} sx={{ color: "#888", "&:hover": { color: "#2e7d32", bgcolor: "#e3f0e8" } }}>
                            <WhatsAppIcon sx={{ fontSize: 17 }} />
                          </IconButton>
                          <IconButton size="small" onClick={() => setSlips(p => p.filter(s => s.id !== sl.id))} sx={{ color: "#888", "&:hover": { color: "#c62828", bgcolor: "#fce4e4" } }}>
      ...
      <Dialog 
        ...
        maxWidth="md" 
        fullWidth
        sx={{ "& .MuiDialog-paper": { borderRadius: "20px", overflow: "hidden", maxHeight: "90vh" } }}
      >
        {/* Header */}
        <Box sx={{ 
          background: "linear-gradient(135deg, #c86030 0%, #a8502a 100%)", 
          ...
          <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
            {companyInfo.logoUrl ? (
              <Box component="img" src={companyInfo.logoUrl} alt="Logo" sx={{ width: 64, height: 64, borderRadius: "12px", objectFit: "cover", bgcolor: "rgba(255,255,255,0.15)" }} />
            ) : (
              <Box sx={{ width: 64, height: 64, borderRadius: "12px", bgcolor: "rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 700, fontFamily: "Josefin Sans, sans-serif" }}>
                CC1
              </Box>
            )}
            <Box>
              <Typography sx={{ fontSize: 11, opacity: 0.8, letterSpacing: "0.1em", textTransform: "uppercase", mb: 0.5, fontFamily: "Josefin Sans, sans-serif" }}>
                RESTAURANT / COMPANY
              </Typography>
              <Typography sx={{ fontSize: 18, fontWeight: 700, fontFamily: "Josefin Sans, sans-serif" }}>
                {companyInfo.name}
              </Typography>
            </Box>
          </Box>
          <Box sx={{ textAlign: "right" }}>
            <Typography sx={{ fontSize: 13, mb: 1, fontFamily: "Josefin Sans, sans-serif" }}>Payslip For The Month</Typography>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, bgcolor: "rgba(255,255,255,0.15)", borderRadius: "10px", px: 2, py: 1 }}>
              <FormControl variant="standard" sx={{ minWidth: 100 }}>
                <Select value={selectedMonth} onChange={(e) => setSelectedMonth(Number(e.target.value))} sx={{ color: "#fff", fontSize: 14, fontWeight: 600, fontFamily: "Josefin Sans, sans-serif", "& .MuiSelect-icon": { color: "#fff" }, "&:before": { borderBottom: "none" }, "&:after": { borderBottom: "none" }, "&:hover:not(.Mui-disabled):before": { borderBottom: "none" } }}>
                  {Array.from({ length: 12 }, (_, i) => (
                    <MenuItem key={i + 1} value={i + 1} sx={{ fontFamily: "Josefin Sans, sans-serif" }}>{getMonthName(i + 1)}</MenuItem>
                  ))}
                </Select>
              </FormControl>
              <Typography sx={{ fontSize: 14, fontWeight: 600, fontFamily: "Josefin Sans, sans-serif" }}>,</Typography>
              <FormControl variant="standard" sx={{ minWidth: 80 }}>
                <Select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} sx={{ color: "#fff", fontSize: 14, fontWeight: 600, fontFamily: "Josefin Sans, sans-serif", "& .MuiSelect-icon": { color: "#fff" }, "&:before": { borderBottom: "none" }, "&:after": { borderBottom: "none" }, "&:hover:not(.Mui-disabled):before": { borderBottom: "none" } }}>
                  {[2024, 2025, 2026, 2027, 2028].map(year => (
                    <MenuItem key={year} value={year} sx={{ fontFamily: "Josefin Sans, sans-serif" }}>{year}</MenuItem>
                  ))}
                </Select>
              </FormControl>
              <CalendarTodayIcon sx={{ fontSize: 18 }} />
            </Box>
            <Typography sx={{ fontSize: 12, mt: 0.5, opacity: 0.8, fontFamily: "Josefin Sans, sans-serif" }}>
              {String(selectedMonth).padStart(2, "0")} - {getMonthName(selectedMonth)} {selectedYear}
            </Typography>
          </Box>
        </Box>
        ...
        <DialogContent sx={{ p: 3, bgcolor: "#faf9f7" }}>
          {/* Employee Selection */}
          <Box sx={{ mb: 3 }}>
            <Typography sx={{ fontSize: 13, fontWeight: 600, color: "#1a1a1a", mb: 1, fontFamily: "Josefin Sans, sans-serif" }}>
          ...
          <Box sx={cardSx}>
            ...
            <Box sx={{ mb: 3 }}>
              <Typography sx={{ fontSize: 12, color: "#666", mb: 0.5, fontFamily: "Josefin Sans, sans-serif" }}>
              ...
              <TextField 
                ...
                sx={{ 
                  ...inputSx, 
                  "& .MuiOutlinedInput-root": { ...inputSx["& .MuiOutlinedInput-root"], bgcolor: "#f0f0f0" } 
                }} 
              />
            </Box>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, mb: 2 }}>
              <TextField 
                ...
                disabled 
                sx={{ ...inputSx, "& .MuiOutlinedInput-root": { ...inputSx["& .MuiOutlinedInput-root"], bgcolor: "#f0f0f0" } }} 
              />
              <TextField 
                label="Employee ID" 
          ...
          <Box sx={cardSx}>
            ...
            <Typography sx={subtextSx}>
              Load overtime for the selected employee in a date range, then add total hours to the slip. Amount is calculated from basic salary and working hours per day.
            </Typography>
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, mb: 2 }}>
              <TextField 
                label="Start date" 
          ...
          <Box sx={cardSx}>
            ...
            <Typography sx={subtextSx}>
              Load late entries for the selected employee in a date range. You can add a late deduction to the slip if needed.
            </Typography>
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, mb: 2 }}>
              <TextField 
                label="Start date" 
          ...
          <Box sx={cardSx}>
            ...
            {/* Basic Salary */}
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2, pb: 2, borderBottom: "1px solid #f0ede8" }}>
              <Typography sx={{ flex: 1, fontSize: 14, fontFamily: "Josefin Sans, sans-serif" }}>Basic Salary</Typography>
            ...
            {allowances.map((allowance, index) => (
              <Box key={allowance.id} sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2, pb: 2, borderBottom: index < allowances.length - 1 ? "1px solid #f0ede8" : "none" }}>
                <TextField 
                ...
                />
                {allowances.length > 1 && (
                  <IconButton onClick={() => removeAllowance(allowance.id)} sx={{ color: "#ef4444", "&:hover": { bgcolor: "#fef2f2" } }}>
          ...
          <Box sx={cardSx}>
            ...
            {deductions.map((deduction, index) => (
              <Box key={deduction.id} sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2, pb: 2, borderBottom: index < deductions.length - 1 ? "1px solid #f0ede8" : "none" }}>
                <TextField 
                ...
                />
                {deductions.length > 1 && (
                  <IconButton onClick={() => removeDeduction(deduction.id)} sx={{ color: "#ef4444", "&:hover": { bgcolor: "#fef2f2" } }}>
                    <RemoveCircleOutlineOutlined />
                  </IconButton>
                )}
              </Box>
          ...
          <Box sx={{ ...cardSx, bgcolor: "#f0fdf4", borderColor: "#bbf7d0" }}>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, mb: 3 }}>
              <Box>
                <Typography sx={{ fontSize: 12, color: "#666", mb: 0.5, fontFamily: "Josefin Sans, sans-serif" }}>Paid Days</Typography>
                <TextField 
                  type="number"
                  value={paidDays}
              ...
              <Box>
                <Typography sx={{ fontSize: 12, color: "#666", mb: 0.5, fontFamily: "Josefin Sans, sans-serif" }}>LOP Days</Typography>
                <TextField 
                  type="number"
                  value={lopDays}
            ...
            <Box sx={{ borderTop: "1px solid #bbf7d0", pt: 2 }}>
              <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1.5 }}>
                <Typography sx={{ fontSize: 14, color: "#374151", fontFamily: "Josefin Sans, sans-serif" }}>Gross Salary</Typography>
                <Typography sx={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", fontFamily: "Josefin Sans, sans-serif" }}>
                  {f(salarySummary.grossSalary)}
                </Typography>
              </Box>
              <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
                <Typography sx={{ fontSize: 14, color: "#374151", fontFamily: "Josefin Sans, sans-serif" }}>Total Deduction</Typography>
                <Typography sx={{ fontSize: 14, fontWeight: 600, color: "#ef4444", fontFamily: "Josefin Sans, sans-serif" }}>
                  {f(salarySummary.totalDeduction)}
                </Typography>
              </Box>
              <Box sx={{ display: "flex", justifyContent: "space-between", pt: 2, borderTop: "1px solid #86efac" }}>
                <Typography sx={{ fontSize: 16, fontWeight: 600, color: "#047857", fontFamily: "Josefin Sans, sans-serif" }}>Net Salary</Typography>
                <Typography sx={{ fontSize: 18, fontWeight: 700, color: "#047857", fontFamily: "Josefin Sans, sans-serif" }}>
          ...
          {/* Action Buttons */}
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pt: 2, borderTop: "1px solid #eae7e2" }}>
      ...
      >
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#faf9f7", borderBottom: "1px solid #eae7e2" }}>
          <Typography sx={{ fontSize: 16, fontWeight: 600, fontFamily: "Josefin Sans, sans-serif" }}>
            Salary Slip Preview
          </Typography>
