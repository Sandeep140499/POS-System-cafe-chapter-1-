  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Top customers ranked by total spend."
      />
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 2, mb: 3 }}>
        {[
          ...
          {
            label: 'TOTAL REVENUE',
            value: `₹${data.reduce((s, r) => s + r.spent, 0).toLocaleString('en-IN')}`,
          },
        ].map(s => (
          <Box
            key={s.label}
                ...
                {ranked.map(r => (
                  <TableRow
                    ...
                    sx={{
                      ...
                      bgcolor: r.rank <= 3 ? 'rgba(200,96,48,0.02)' : 'transparent',
                    }}
                  >
                    <TableCell sx={cSx}>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2 }}>
                        <Avatar
                          sx={{
                            width: 32,
                            height: 32,
                            bgcolor: COLORS[(r.rank - 1) % COLORS.length],
                            fontSize: 12,
                            fontWeight: 700,
                          }}
                        >
                    ...
                    <TableCell sx={cSx}>
                      <Typography
                        sx={{ fontSize: 14, color: '#666', fontFamily: 'Inter, sans-serif' }}
                      >
                        {r.mobile}
                      </Typography>
                    </TableCell>
                    ...
                    <TableCell sx={{ ...cSx, textAlign: 'center' }}>
                      <IconButton
                        size="small"
                        onClick={() => handleDeleteCustomer(r.id)}
