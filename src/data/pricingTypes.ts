...
export function addPricingType(
  type: Omit<CustomPricingType, 'id' | 'createdAt' | 'isBuiltIn'>
): CustomPricingType {
...
export function updatePricingType(
  id: string,
  updates: Partial<CustomPricingType>
): CustomPricingType | null {
  const current = getPricingTypes();
