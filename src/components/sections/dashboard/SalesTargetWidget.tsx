...
export default function SalesTargetWidget() {
  ...
  const statusLabel = reached >= 100 ? "ACHIEVED" : reached >= 75 ? "ON TRACK" : reached >= 50 ? "BEHIND" : "CRITICAL";
  const statusColor = reached >= 100 ? "#2e7d32" : reached >= 75 ? "#558b2f" : reached >= 50 ? "#f57c00" : "#c62828";
  const statusBg = reached >= 100 ? "#e3f0e8" : reached >= 75 ? "#e8f5e9" : reached >= 50 ? "#fff8e1" : "#fce4e4";

  const dailyPace = daysLeft > 0 && targetRaw > collectedRaw
    ? Math.ceil((targetRaw - collectedRaw) / daysLeft)
    : 0;
  ...
  return (
    <SectionPaperCard>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 1 }}>
        <Box>
          <Typography sx={{ fontSize: 10.5, fontWeight: 700, color: "#c86030", letterSpacing: "0.08em", textTransform: "uppercase", mb: 0.3 }}>
            SALES TARGET
          </Typography>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 2, mt: 2.5, mb: 1.5 }}>
        {[
          ...
          <Box key={s.label}>
            <Typography sx={{ fontSize: 9.5, fontWeight: 600, color: "#999", letterSpacing: "0.06em", textTransform: "uppercase", mb: 0.3 }}>
              {s.label}
            </Typography>
      ...
      <LinearProgress
        ...
        sx={{
          ...
          bgcolor: "#eeedea",
          mb: 1.5,
          "& .MuiLinearProgress-bar": { bgcolor: reached >= 100 ? "#2e7d32" : "#c86030", borderRadius: 3 },
        }}
      />
