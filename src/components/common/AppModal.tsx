    ...
    >
      ...
      >
        <Box>
          ...
          {subtitle && (
            <Typography
              sx={{ fontSize: 13, color: '#888', mt: 0.3, fontFamily: 'Inter, sans-serif' }}
            >
              {subtitle}
            </Typography>
          )}
        </Box>
        <IconButton onClick={onClose} size="small" sx={{ mt: -0.5, color: '#999' }}>
          <CloseRoundedIcon sx={{ fontSize: 20 }} />
        </IconButton>
      </DialogTitle>
      <DialogContent sx={{ px: 3, pt: 2, pb: 0 }}>{children}</DialogContent>
      {actions && (
        <DialogActions sx={{ px: 3, py: 2.5, gap: 1, borderTop: '1px solid #eae7e2', mt: 2 }}>
          {actions}
        </DialogActions>
      )}
    </Dialog>
